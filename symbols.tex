...
\symbolindex{$P ⇔ Q$	}{logical equivalence, \pg{defn:logical-notation}}
...
\symbolindex{$x ≤ y$	}{non-strict inequality on natural numbers, \pg{leq-nat}, Cauchy reals, \pg{leq-RC}, surreals, \pg{defn:surreals}, etc.}
\symbolindex{$⪯$, $≺$	}{recursive versions of $\le$ and $<$ for surreals, \pg{defn:No-codes}}
\symbolindex{$\ble$, $\blt$, $⊑$, $\bblt$	}{orderings on codomain of $\NO$-recursion, \pg{NO-recursion}}
...
\symbolindex{$x ↦ b$	}{alternative notation for $\lambda$-abstraction, \pg{mapsto}}
