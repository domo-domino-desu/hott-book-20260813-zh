...
第三，我们的综合方法非常适合利用 \Coq 和 \Agda 等\index{proof!assistant}证明辅助工具进行计算机验证。
本章所描述的证明，几乎全部都已通过计算机验证；其中许多证明最初是在证明辅助工具中给出的，而后才为本书"去形式化"。
...
在这部分注记里，我们回顾这些定理在同伦类型论中新的综合证明是如何发展起来的。
...
除非另有说明，对于已经过计算机验证的定理，其主要作者也是率先使用计算机证明辅助工具将证明\index{mathematics!formalized}形式化的人。
