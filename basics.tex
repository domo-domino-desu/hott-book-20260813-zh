...
正如在 \cref{cha:typetheory} 的附注中提到的，我们这里使用的相等类型属于\emph{内涵}类型论（intensional type theory），而非\emph{外延}类型论（extensional type theory）。
一般来说，如果某种相等性概念根据对象的具体定义方式来区分对象，我们就称其为“内涵的（intensional）”；如果它不区分具有相同“外延”或“可观察行为”的对象，则称其为``外延的（extensional）''。
...
诸如 $\transfib{P}{p}{\blank}$ 与 $\apfunc{f}$ 等操作，以及一种恰当的等价概念，最初由 Voevodsky 使用证明助理 \Coq 在类型论中加以深入研究。
