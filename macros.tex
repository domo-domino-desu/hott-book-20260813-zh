...
\makeatletter
\newcommand{\sectionNotes}{\phantomsection\section*{附注}\addcontentsline{toc}{section}{附注}\markright{\textsc{\@chapapp{} \thechapter{} 附注}}}
...
%%% Term being defined
% 中译本改动：中文排版里正文强调惯例上不是把宋体加粗，而是换用黑体；
% 标题的粗体不受影响。假定只用 LuaLaTeX 编译。
\setCJKfamilyfont{zhemph}{FandolSong-Regular}[
  Extension = .otf,
...
\renewcommand{\textbf}[1]{{\CJKfamily{zhemph}\bfseries #1}}
...
% 中译本改动：环境标题（\newtheorem 第二个参数）和 cleveref 的 \cref 输出
% （\crefname 后两个参数）都改成中文，让"定理 3.2.1"这类自动生成的标题和
% 交叉引用文字跟已经译成中文的正文保持一致，不再是中英夹杂。
% 中译本改动：amsthm 环境标题（``引理 3.1''这类）默认用 \bfseries 加粗，
% 但正文强调已经统一改用 zhemph family（黑体）而非宋体假粗体（见上面
% \renewcommand{\textbf}）；这里让 thm/cor/lem/axiom/defn 这些加粗标题的
% 环境也走同一套字体，保持全书粗体观感一致。remark 样式（注记/例/习题等）
% 用的是斜体而非加粗，不受影响，故不改。
\thm@headfont{\CJKfamily{zhemph}\bfseries}
...
% 中译本改动：中文习惯"第 3.2 节"这种数字被前后缀包住的格式，跟英文纯前缀的
% "Section 3.2"不同，所以这里连位置参数的顺序都要跟着调整，不只是换词。
% 大小写形式（cref/Cref）在中文里没有区别，两个宏给一样的格式。
...
\Crefmultiformat{section}{第~#2#1#3}{~和~#2#1#3~节}{、#2#1#3}{~和~#2#1#3~节}
\crefrangemultiformat{section}{第~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~节}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~节}
\Crefrangemultiformat{section}{第~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~节}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~节}

% Display format for appendices
\crefformat{appendix}{附录~#2#1#3}
\Crefformat{appendix}{附录~#2#1#3}
\crefrangeformat{appendix}{附录~#3#1#4~至~#5#2#6}
\Crefrangeformat{appendix}{附录~#3#1#4~至~#5#2#6}
\crefmultiformat{appendix}{附录~#2#1#3}{~和~#2#1#3}{、#2#1#3}{~和~#2#1#3}
\Crefmultiformat{appendix}{附录~#2#1#3}{~和~#2#1#3}{、#2#1#3}{~和~#2#1#3}
\crefrangemultiformat{appendix}{附录~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6}
\Crefrangemultiformat{appendix}{附录~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6}
...
% 中译本改动：cleveref 对 chapter/table/equation 有自己内置的默认英文名
% （不跟着 \chaptername 这类 babel/polyglossia 的本地化走），原书从没显式
% 覆盖过，所以 \cref{cha:xxx} 这类引用一直在用英文 "Chapter" 渲染，跟已经
% 译成中文的正文不一致，这里补上。
\crefname{chapter}{章}{章}
\crefname{table}{表}{表}
\crefname{equation}{式}{式}

% 光有 \crefname 只解决了单条 \cref{cha:xxx} 的名词翻译，多条引用
% （\cref{cha:a,cha:b}）走的是 \crefmultiformat，cleveref 没给它设中文
% 格式时会退回内置的英文连接词 "and"，所以补上跟 section 一致的格式。
\crefformat{chapter}{第~#2#1#3~章}
\Crefformat{chapter}{第~#2#1#3~章}
\crefrangeformat{chapter}{第~#3#1#4~至~#5#2#6~章}
\Crefrangeformat{chapter}{第~#3#1#4~至~#5#2#6~章}
\crefmultiformat{chapter}{第~#2#1#3}{~和~#2#1#3~章}{、#2#1#3}{~和~#2#1#3~章}
\Crefmultiformat{chapter}{第~#2#1#3}{~和~#2#1#3~章}{、#2#1#3}{~和~#2#1#3~章}
\crefrangemultiformat{chapter}{第~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~章}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~章}
\Crefrangemultiformat{chapter}{第~#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~章}{、#3#1#4~至~#5#2#6}{~和~#3#1#4~至~#5#2#6~章}

% 上面这些 \crefmultiformat 只覆盖"同类型"多重引用（比如两个 section 一起
% 引用）。但书里更常见的是"跨类型"多重引用，例如 \cref{sec:x,cha:y}
% （一个 section 一个 chapter 混在一条 \cref 里），这种情况 cleveref 走的
% 是另一套机制——\crefpairconjunction 等连接词宏，而不是 crefmultiformat，
% 这套宏没有任何中文本地化包提供翻译，一直在退回内置英文 "and"/","，是
% "第 2.9 和 2.10 节 and 章 6"这类中英夹杂的真正根源。这里统一改成中文
% 顿号／"和"，一次性修好所有类型组合下的多重引用连接词。
...
\newcommand*{\crefrangepregroupconjunction}{、}
\newcommand*{\crefrangepostgroupconjunction}{}
