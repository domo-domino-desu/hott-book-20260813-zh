...
我们引入原始常量 $c_\Sigma$ 和 $c_{\mathsf{pair}}$。
形如 $c_\Sigma(A,\lam{a} B)$ 的表达式记作 $\sm{a:A}B$，形如 $c_{\mathsf{pair}}(a,b)$ 的表达式记作 $\tup a b$。
若 $x$ 不在 $B$ 中自由出现，我们就把 $\sm{x:A} B$ 写作 $A\times B$。
...
%
设 $A$ 与 $B$ 如上，$C : (\sm{x:A}B(x)) \rightarrow \UU_m$，且
\[
...
\]
%
注意，若 $C$、$d$、$x$、$y$ 是在某个非空上下文中得到的，它们可能带有额外的隐式参数 $x_1,\ldots,x_n$；因此，完全显式的递归模式为
%
